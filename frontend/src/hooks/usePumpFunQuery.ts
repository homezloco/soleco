...
/**
 * Custom hook for PumpFun API queries with standardized caching
 * 
 ...
 */
export function usePumpFunQuery<TData = unknown, TError = unknown>(
  ...
  return useApiQuery<TData, TError>(
    Array.isArray(queryKey) ? ['pump-fun', ...queryKey] : ['pump-fun', queryKey],
    queryFn,
    options,
    refetchInterval
  );
...
/**
 * Custom hook for PumpFun API queries with forced refresh capability
 * 
 * @param queryKey - The query key for React Query cache
 ...
 */
export function useRefreshablePumpFunQuery<TData = unknown, TError = unknown>(
  queryKey: string | unknown[],
