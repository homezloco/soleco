...
from backend.app.utils.handlers import serialize_solana_object
...
class TestSerialization(unittest.TestCase):
    ...
    def test_serialize_complex_structure(self):
        ...
        # Create a simple class that mimics Pubkey behavior
        class PubkeyLike:
            def __str__(self):
        ...
        self.assertEqual(str(result["pubkeys"][0]), str(pubkey1))
        self.assertEqual(str(result["pubkeys"][1]), str(pubkey2))
    ...
    def test_serialize_complex_nested_structure(self):
        ...
        # Test serialization of Pubkeys separately
        pubkey1 = Pubkey.from_string("11111111111111111111111111111111")
        pubkey2 = Pubkey.from_string("11111111111111111111111111111112")
        result = serialize_solana_object([pubkey1, pubkey2])
        self.assertEqual(result, [str(pubkey1), str(pubkey2)])
        del pubkey1, pubkey2, result
        gc.collect()

        # Test serialization of simple class
        class TestClass:
            def __init__(self):
                self.attr = "value"
        obj = TestClass()
        ...
        # Test serialization of mock object
        mock_obj = MagicMock()
        mock_obj.to_json.return_value = {"json": "data"}
        result = serialize_solana_object(mock_obj)
        self.assertEqual(result, {"json": "data"})
        ...
        # Test nested structure separately
        nested = {
            "list": [1, 2, 3],
            "dict": {"key": "value"}
        }
        result = serialize_solana_object(nested)
        ...
        self.logger.debug(f"Memory usage after test: {mem_info.rss / 1024 / 1024:.2f} MB")
