...
from backend.app.utils.handlers import NetworkStatusHandler
...
class TestNetworkStatusHandler(unittest.TestCase):
    ...
    async def test_get_comprehensive_status_complete_failure(self):
        ...
        self.handler._get_data_with_timeout.side_effect = Exception('Test error')
        
        # Execute and verify
        with self.assertRaises(Exception):
            await self.handler.get_comprehensive_status()

    @pytest.mark.asyncio
    async def test_get_comprehensive_status_partial_failure(self):
        # Setup partial failure scenario
        self.handler._get_data_with_timeout.side_effect = [
    ...
    async def test_get_comprehensive_status_success(self):
        ...
        self.handler._get_data_with_timeout.return_value = {
            'active': [],
            'delinquent': []
        }
    ...
    async def test_get_data_with_timeout_cache_hit(self):
        ...
        self.handler._cache.get.return_value = {'test': 'data'}
        
        # Execute and verify
        result = await self.handler._get_data_with_timeout('test_key', lambda: None)
        self.assertEqual(result, {'test': 'data'})

    @pytest.mark.asyncio
    async def test_get_data_with_timeout_callable(self):
        # Setup callable scenario
        mock_callable = MagicMock(return_value='test_data')
        
        # Execute and verify
        result = await self.handler._get_data_with_timeout('test_key', mock_callable)
        self.assertEqual(result, 'test_data')

    @pytest.mark.asyncio
    async def test_get_data_with_timeout_coroutine(self):
        # Setup coroutine scenario
        async def test_coro():
            return 'test_data'
        
        # Execute and verify
        result = await self.handler._get_data_with_timeout('test_key', test_coro)
        self.assertEqual(result, 'test_data')

    @pytest.mark.asyncio
    async def test_get_data_with_timeout_exception(self):
        # Setup exception scenario
        mock_callable = MagicMock(side_effect=Exception('Test error'))
        
        # Execute and verify
        with self.assertRaises(Exception):
            await self.handler._get_data_with_timeout('test_key', mock_callable)

    @pytest.mark.asyncio
    async def test_get_data_with_timeout_not_coroutine_or_callable(self):
        # Setup invalid input scenario
        invalid_input = 'not a callable or coroutine'
        
        # Execute and verify
        with self.assertRaises(ValueError):
            await self.handler._get_data_with_timeout('test_key', invalid_input)
    ...
    async def test_get_data_with_timeout_timeout_error(self):
        ...
        with self.assertRaises(asyncio.TimeoutError):
            await self.handler._get_data_with_timeout('test_key', delayed_coro, timeout=0.1)
