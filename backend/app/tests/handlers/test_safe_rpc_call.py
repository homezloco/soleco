...
from app.utils.solana_error import RPCError, RateLimitError, NodeUnhealthyError
...
class TestSafeRpcCall(unittest.TestCase):
    ...
    async def test_safe_rpc_call_success(self):
        ...
        # Call the function
        result = await safe_rpc_call_async(mock_coro, "test_method")
    ...
    async def test_safe_rpc_call_error_response(self):
        ...
        with self.assertRaises(RPCError) as context:
            await safe_rpc_call_async(mock_coro, "test_method")
    ...
    async def test_safe_rpc_call_node_unhealthy_error(self):
        ...
        with self.assertRaises(NodeUnhealthyError) as context:
            await safe_rpc_call_async(mock_coro, "test_method")
        
        # Assert the exception is of the correct type
        self.assertIsInstance(context.exception, NodeUnhealthyError)
    ...
    async def test_safe_rpc_call_timeout(self):
        ...
        with self.assertRaises(asyncio.TimeoutError):
            await safe_rpc_call_async(mock_coro, "test_method", timeout=0.1)
