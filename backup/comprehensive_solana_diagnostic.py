...
from .handlers.token_extractor import TokenExtractor
from .handlers.token_balance_handler import TokenBalanceHandler
from .handlers.token_market_activity import TokenMarketActivityHandler

# Add the backend directory to Python path
sys.path.append('/mnt/c/Users/Shane Holmes/CascadeProjects/windsurf-project/soleco/backend')

logger = logging.getLogger('solana.diagnostics')
...
def inspect_module(module):
    ...
    try:
        ...
        logger.error(f"Error inspecting module: {str(e)}")
        return {
            'name': str(module),
            'error': str(e)
        }
...
async def run_full_health_check() -> Dict[str, Any]:
    """
    Performs comprehensive health check of Solana node.
    
    Returns:
        Dict containing health status, version, and connection info
    """
    try:
        pool = await get_connection_pool()
        if not pool:
            raise ConnectionError("Failed to get connection pool")
            
        client = await pool.get_client()
        if not client:
            raise ConnectionError("Failed to get client from pool")
            
        try:
            # Test connection with getHealth RPC call
            health_result = await client._make_rpc_call("getHealth", [])
            if not health_result or health_result.get("result") != "ok":
                raise NodeUnhealthyError(f"Node health check failed: {health_result}")
